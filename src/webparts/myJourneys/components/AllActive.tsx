...
export interface IAllActiveProps {
  journeys: IHistory[];
  sp: SPFI;
}
...
export const AllActive: React.FC<IAllActiveProps> = ({ journeys, sp }) => {
  ...
  if (journeys.length === 0) {
    return (
      <div className={styles.emptyState}>
        <Text size={400} weight="semibold">No active journeys</Text>
        <Text size={200}>Active journeys across the site will appear here</Text>
      </div>
    );
  }
  ...
  const columns: TableColumnDefinition<IHistory>[] = [
    ...
    createTableColumn<IHistory>({
      ...
      renderHeaderCell: () => 'Initiated By',
      renderCell: (item) => <Text>{item.InitiatedBy}</Text>,
    }),
    createTableColumn<IHistory>({
      columnId: 'status',
  ...
  return (
    <>
      <DataGrid items={journeys} columns={columns} getRowId={(item) => String(item.Id)}>
        <DataGridHeader>
          <DataGridRow>
