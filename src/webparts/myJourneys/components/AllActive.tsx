...
export const AllActive: React.FC<IAllActiveProps> = ({ searchQuery, statusFilter, refreshKey }) => {
  ...
  const loadData = async (): Promise<void> => {
    setLoading(true);
    try {
      const journeys = await journeyService.getAllActiveJourneys();
      setItems(journeys);
    } catch {
      // Handle silently
    }
  ...
  React.useEffect(() => { loadData().catch(() => {}); }, [refreshKey]);
  ...
  const columns: TableColumnDefinition<IHistory>[] = [
    ...
    createTableColumn<IHistory>({
      ...
      renderCell: (item) => (
        <Text>User #{item.InitiatedById}</Text>
      ),
    }),
    createTableColumn<IHistory>({
      columnId: 'started',
  ...
  if (items.length === 0) {
    return (
      <div className={styles.empty}>
        <Text size={400}>No active journeys</Text>
      </div>
    );
  }
